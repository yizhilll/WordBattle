...
2. 挑战接收后开始对战。
3. 服务器记录单词，由客户端A获取，倒数5秒后单词消失开始复现，不限尝试次数。
...
2. 对战
   `发起方userA|发起方得分|接受方|接受方得分（-1未接受挑战）|当前对局状态 state|当前第几轮|当前单词|当前局发起方完成时间|当前局接受方完成时间`

   `state`字段： -1 等待A出单词；0  A已经出单词，等待回答；2 A已经回答，等待B；4 B等待A；8 当前局完成，等待 A判定。

用 `|` 作分隔符发送服务器端和用户的消息。

服务器端信息设计：`username|online/offline`

#### 服务端任务划分：

1. 相应用户登录，并记录。
2. 相应用户下线，并记录。
3. 检查用户在线：生存时间/对战请求失败/关闭窗口动作读取。
4. 记录在线用户发起对战
5. 记录实时对战的信息
...
- 发起者A在 `onbattle` 表格创建记录向服务器记录发起挑战，将 `scoreA` 设为0。
  - 然后定时扫描 `scoreB` 是否变为0 （B接受挑战）。弹出对战窗口，进入对战状态。
  - 同时判定 `scoreA` 是否被修改为 -1 （B拒绝挑战），如果有，则删除整条记录。
- 接受方B定时扫描 `onbattle` ，查看是否有自己的用户名，并且 `scoreB` 为 -1 。如果有，则给用户弹窗，询问是否接受。
  - 如果接受，将记录 `scoreB` 修改 为 0，并且进入对战窗口。
  - 如果拒绝，则修改 `scoreA` 为 -1。

**客户端对战处理**（周期 1s）：

- 做题：
  - 从服务器获取 `currentword` 初始化
  - 倒数 5s 显示单词
  - 倒数 30 s 答题。
  - 答题正确则记录时间，发送到服务器；如果失败则发30s。发送时间同时修改状态
    - 如果是A： `state` 为0 则改为2；state 为4 则改为8。
    - 如果是B： `state` 为0 则改为4；state 为2 则改为8。
- 发起者A **不做题的时候**定期检查记录的 `state` 
  - -1 ： 随机出题，修改`state` 为0，修改 `currentword`，`currentround` +1，并且开始做题
  - 4 ：发现B已经答题完毕，
  - 2 ：继续等
  - 8 ：比较 `timeA` 和 `timeB`，给胜利方加 1 分。当 `currentround` 为5 时，判定胜利方。如果没有到5，则等待状态10，
  - 10：B已经读出结果，A删除记录并退出。
- 接受者B **不做题的时候**定期检查记录的 `state` ：
  - -1 ：等待 A 出题，如果检测到在线state 为 0 则修改本地状态。
  - 0 ：获取 `currentword` 开始做题
  - 2 ：A已经完成，等待B在答题
  - 4 ：等待A答题
  - 8 ：读取比分并且修改。进入状态 10，告知A已经读取。当第五轮结束时，将判定结果通知用户。
...
`g++ wbserver.cpp -I/usr/include/mysql -o wbserver -lmysqlclient`